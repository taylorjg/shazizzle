...
const drawCharts = async (sampleRate, fftSize, testTone) => {
  try {
    ...
    // I am using 2048 for 'begin' here because the early
    // part of the mp3 files seems to contain zeros.
    const begin = 2048
    const end = begin + fftSize
    const channelData = audioBuffer.getChannelData(0).slice(begin, end)
